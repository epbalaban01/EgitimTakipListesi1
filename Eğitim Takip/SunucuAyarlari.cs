...
{
    public partial class SunucuAyarlari : Form
    {
        string server = svsettings.Default.server, 
            database = svsettings.Default.database, 
            username = svsettings.Default.username, 
           password = svsettings.Default.password;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (serverText.Text.Trim() != "" && databaseText.Text.Trim() != "" && usernameText.Text.Trim() != "")
            {
                try
                {
                    SqlConnection con = new SqlConnection("Server=" + serverText.Text.Trim() + ";Database=" + databaseText.Text.Trim() + ";Uid=" + usernameText.Text.Trim() + ";Pwd=" + passwordText.Text.Trim() + ";");
                    con.Open();
                    MessageBox.Show("Sunucuyla başarılı bir şekilde bağlantı kuruldu.");
                    svsettings.Default.server = serverText.Text.Trim();
                    svsettings.Default.database = databaseText.Text.Trim();
                    svsettings.Default.username = usernameText.Text.Trim();
                    svsettings.Default.password = passwordText.Text.Trim();
        ...
        {
            SqlConnection con = new SqlConnection("Server=" + server + ";Database=" + database + ";Uid=" + username + ";Pwd=" + password + ";");
